  ...
  return (
    <div className="p-6">
      ...
      <div className="relative mb-4">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="이름, 전화번호, 이메일 검색"
      ...
      <div className="grid gap-3">
        {filtered.map((student) => (
          <div key={student.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-between hover:shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  {student.phone && <span className="flex items-center gap-1 text-xs text-gray-500"><Phone className="w-3 h-3" />{student.phone}</span>}
                  {student.email && <span className="flex items-center gap-1 text-xs text-gray-500"><Mail className="w-3 h-3" />{student.email}</span>}
                </div>
              </div>
            </div>
        ...
        {filtered.length === 0 && <div className="text-center py-12 text-gray-400">{search ? "검색 결과가 없어요" : "등록된 학생이 없어요"}</div>}
      </div>
      {isModalOpen && (
