...
const methodLabel: Record<string, string> = { cash: "현금", transfer: "계좌이체", card: "카드" };
const methodColor: Record<string, string> = { cash: "bg-green-100 text-green-700", transfer: "bg-blue-100 text-blue-700", card: "bg-purple-100 text-purple-700" };
...
export default function PaymentsPage() {
  ...
  async function fetchPayments() {
    ...
    const { data } = await supabase.from("payments").select("*, students(name)")
      .gte("payment_date", start).lte("payment_date", end).order("payment_date", { ascending: false });
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="px-3 py-1 text-sm font-medium text-gray-700">{format(currentDate, "yyyy년 M월", { locale: ko })}</span>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4 col-span-2">
          <div className="flex items-center gap-2 mb-1"><TrendingUp className="w-4 h-4 text-sky-500" /><span className="text-sm text-gray-500">이번 달 총 수입</span></div>
        ...
        {Object.entries(byMethod).map(([method, amount]) => (
          <div key={method} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p className="text-lg font-bold text-gray-900 mt-2">{(amount as number).toLocaleString()}원</p>
          </div>
        ))}
      </div>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {payments.map((p) => (
              <tr key={p.id} onClick={() => { setSelectedPayment(p); setIsModalOpen(true); }} className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer">
                ...
                <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${methodColor[p.payment_method]}`}>{methodLabel[p.payment_method]}</span></td>
            ...
            {payments.length === 0 && <tr><td colSpan={5} className="text-center py-12 text-gray-400">이번 달 결제 내역이 없어요</td></tr>}
          </tbody>
        </table>
      </div>
      {isModalOpen && (
